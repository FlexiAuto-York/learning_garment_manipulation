...
# Resolve this repo's location rather than assuming a directory name, so the clone can
# live anywhere and be called anything. Requires `source`ing, not executing.
_MP_REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"

conda activate "${MAGPIE_CONDA_ENV:-magpie}"

# SoftGym (PyFlex) is only needed for simulation. Set SOFTGYM_PATH if it is not a sibling.
_MP_SOFTGYM="${SOFTGYM_PATH:-${_MP_REPO_DIR}/../softgym}"
if [ -d "${_MP_SOFTGYM}" ]; then
  cd "${_MP_SOFTGYM}"
  . ./setup.sh
else
  echo "SoftGym not found at ${_MP_SOFTGYM} - skipping (simulation will not run)."
  echo "  Set SOFTGYM_PATH=/path/to/softgym if it lives elsewhere."
fi
...
unset _MP_SETUP_FLAG _MP_REPO_DIR _MP_SOFTGYM
