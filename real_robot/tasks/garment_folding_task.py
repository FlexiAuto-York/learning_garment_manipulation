...
class RealWorldGarmentCanonAlignFoldingTask(RealWorldGarmentFoldingTask):
    ...
    def evaluate(self, arena):
        ...
        # mirroring success()'s latched branch. Deliberately independent of
        # `has_succeeded`: evaluate() runs before success(), so reading the latched
        # flag here would lag the active subgoal by one step.
